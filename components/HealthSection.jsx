...
const HealthSection = () => {
  ...
  useEffect(() => {
      const getBookCategory = async () => {
        try {
          ...
          const booksInCategory = data.filter(item => item.category === "Health") 
          ...
          console.error('Error fetching Health section:', error)
        }    
      }
  ...
  return (
    <View>
        <Text className='text-2xl text-white font-bold mb-2 px-5'>Be more healthy</Text>
